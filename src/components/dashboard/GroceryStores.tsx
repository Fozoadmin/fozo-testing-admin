...
export function GroceryStores() {
  ...
  const handleCreate = async () => {
    ...
    const bankAccountDetails =
      bank.accountNumber || bank.ifscCode
        ? { accountNumber: bank.accountNumber, ifscCode: bank.ifscCode, accountHolderName: bank.accountHolderName, bankName: bank.bankName }
        : undefined;
    ...
    try {
      const bankAccountDetails =
        editBank.accountNumber || editBank.ifscCode
          ? { accountNumber: editBank.accountNumber, ifscCode: editBank.ifscCode, accountHolderName: editBank.accountHolderName, bankName: editBank.bankName }
          : undefined;
      ...
      toast.success("Grocery store updated successfully!", { position: "top-right", autoClose: 3000 });
      setOpenEdit(false);
      setSelectedStore(null);
      await fetchStores();
    } catch (error: any) {
      toast.error(error.message || "Failed to update store", { position: "top-right", autoClose: 3000 });
    } finally {
      setEditing(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Grocery store deleted successfully!", { position: "top-right", autoClose: 3000 });
      setOpenDelete(false);
      setStoreToDelete(null);
      await fetchStores();
    } catch (error: any) {
      toast.error(error.message || "Failed to delete store", { position: "top-right", autoClose: 3000 });
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {/* ===== ADD STORE DIALOG ===== */}
          <Dialog open={openAdd} onOpenChange={(v) => { setOpenAdd(v); if (!v) resetAddForm(); }}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => { setStoreToDelete(store); setOpenDelete(true); }}
                          >
          ...
          <Tabs defaultValue="basic" className="w-full">
            ...
            <TabsContent value="basic" className="space-y-3 mt-4">
              ...
              <div className="space-y-2">
                ...
                {editForm.imageUrl && (
                  <img src={editForm.imageUrl} alt="preview" className="h-20 rounded object-cover" />
                )}
              </div>
            </TabsContent>

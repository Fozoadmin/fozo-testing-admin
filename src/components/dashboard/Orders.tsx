...
export function Orders() {
  ...
  useEffect(() => {
    ...
    const unsubUpdated = subscribeToEvent<GroceryOrder>(
      SOCKET_EVENTS.GROCERY_ORDER_UPDATED,
      updated => {
  ...
  const handleGroceryStatusChange = async (order: GroceryOrder, newStatus: string) => {
    ...
    try {
      ...
      setSelectedGrocery(sel =>
        sel && sel.id === order.id ? { ...sel, orderStatus: newStatus } : sel
      );
      toast.success(`Status updated to ${groceryStatusLabel(newStatus)}`, {
        position: 'top-right',
        autoClose: 3000,
      });
    } catch (e: any) {
      ...
      {activeTab === 'grocery' ? (
        ...
        <div className='grid gap-4'>
              ...
              <div className='grid grid-cols-3 gap-2'>
                ...
                <div className='rounded-xl border p-3 text-center'>
                  ...
                  <div className='text-lg font-semibold'>
                    {formatINR(
                      groceryOrders.reduce((s, o) => s + Number(o.totalPaymentAmount || 0), 0)
                    )}
                  </div>
                </div>
                <div className='rounded-xl border p-3 text-center'>
          ...
          <Card className='rounded-2xl'>
            ...
            <CardContent>
              {groceryLoading ? (
                ...
                <div className='text-destructive py-8 text-center'>{groceryError}</div>
                    ...
                    <TableBody>
                      {filteredGrocery.map(o => (
                        <TableRow
                          ...
                          tabIndex={0}
                          onKeyDown={e => {
                            if (e.key === 'Enter' || e.key === ' ') setSelectedGrocery(o);
                          }}
                        >
                          ...
                          <TableCell onClick={e => e.stopPropagation()}>
                            ...
                            >
                              <SelectTrigger className='h-7 w-[170px] px-2 text-xs'>
                                {updatingGroceryStatusId === o.id ? (
                                  ...
                                  <Badge
                                    variant={groceryStatusVariant(o.orderStatus)}
          ...
          <Sheet open={!!selectedGrocery} onOpenChange={open => !open && setSelectedGrocery(null)}>
            <SheetContent
              side='right'
              className='w-full overflow-y-auto px-2 sm:max-w-xl lg:max-w-2xl'
            >
              {selectedGrocery && (
                <div className='space-y-4'>
                  ...
                  <Card className='rounded-xl'>
                    ...
                    <CardContent className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
                      ...
                      <InfoRow
                        ...
                        value={selectedGrocery.paymentMethod || '-'}
                      />
                      <InfoRow
                        label='Payment Status'
                        value={selectedGrocery.paymentStatus}
                        icon={
                          selectedGrocery.paymentStatus === 'paid' ? (
                  ...
                  <Card className='rounded-xl'>
                    ...
                    <CardContent className='grid grid-cols-2 gap-3'>
                      ...
                      <MoneyRow
                        ...
                        value={selectedGrocery.platformCommission}
                      />
                      <MoneyRow label='GST' value={selectedGrocery.gstAmount} />
                      {Number(selectedGrocery.discountAmount || 0) > 0 && (
                        <div className='col-span-2 flex items-center justify-between text-sm text-emerald-600'>
                          <span>Discount</span>
                          <span className='font-medium'>
                            -{formatINR(selectedGrocery.discountAmount)}
                          </span>
                        </div>
                      )}
                      ...
                      <MoneyRow
                        ...
                        value={selectedGrocery.totalPaymentAmount}
                        bold
                      />
                    </CardContent>
                  </Card>
                  ...
                  <Card className='rounded-xl'>
                    ...
                    <CardContent className='space-y-2'>
                      {selectedGrocery.items?.length > 0 ? (
                        selectedGrocery.items.map((item, idx) => (
                          <div
                            key={item.id || idx}
                  ...
                  <Card className='rounded-xl'>
                    <CardHeader className='pb-2'>
                      <CardTitle className='text-base'>People & Places</CardTitle>
                    </CardHeader>
                    <CardContent className='grid gap-4'>
                      <div className='grid gap-4 sm:grid-cols-2'>
                        <div className='space-y-1'>
                          ...
                          <div className='text-sm'>
                            {selectedGrocery.customerNameSnapshot ||
                              selectedGrocery.customerName ||
                              '-'}
                          </div>
                          <div className='text-muted-foreground text-xs'>
                            {selectedGrocery.customerPhone || '-'}
                          </div>
                        </div>
                        <div className='space-y-1'>
                          <div className='flex items-center gap-2 text-sm font-medium'>
                      ...
                      <div className='space-y-1'>
                        ...
                        <div className='text-sm leading-5 whitespace-pre-wrap'>
                          {selectedGrocery.deliveryAddressSnapshot || '-'}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            <CardContent className='grid grid-cols-1 gap-3 pt-6 xl:grid-cols-3'>
              ...
              <div className='grid grid-cols-3 gap-2'>
                ...
                <div className='rounded-xl border p-3 text-center'>
                  <div className='text-muted-foreground text-xs'>Delivered</div>
                  <div className='text-lg font-semibold'>
                    {kpis.counts[ORDER_STATUS.DELIVERED] ?? 0}
                  </div>
                </div>
              </div>
            </CardContent>
                              ...
                              <div className='truncate'>
                                ...
                                <a
                                  ...
                                  className={cn(
                                    ...
                                    !o.customerPhone && 'pointer-events-none'
                                  )}
                ...
                <div className='space-y-4'>
                      ...
                      <DialogContent>
                        ...
                        <div className='grid gap-3 py-2'>
                          <div className='grid gap-1'>
                            ...
                            <Select value={dpSelectedId} onValueChange={setDpSelectedId}>
                              <SelectTrigger id='dp'>
                                <SelectValue
                                  placeholder={dpLoading ? 'Loading…' : 'Choose partner'}
                                />
                              </SelectTrigger>
                              <SelectContent>
                                {dpError && (
                        ...
                        <DialogFooter>
                          <Button
                            ...
                            disabled={!dpSelectedId || updatingStatusId !== null}
                            className='gap-2'
                          >
                  ...
                  <Card className='rounded-xl'>
                    ...
                    <CardContent className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
                      ...
                      <InfoRow
                        label='Txn ID'
                        value={selected.paymentTransactionId || '-'}
                        copyable
                      />
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className='grid grid-cols-2 gap-3'>
                      ...
                      {Number(selected.walletAmountUsed || 0) > 0 && (
                        <div className='col-span-2 flex items-center justify-between text-sm text-emerald-600'>
                          ...
                          <span className='font-medium'>
                            -{formatINR(selected.walletAmountUsed)}
                          </span>
                        </div>
                      )}
                      <Separator className='col-span-2' />
                    ...
                    <CardContent className='grid gap-4'>
                      <div className='grid gap-4 sm:grid-cols-2'>
                        <div className='space-y-1'>
                          ...
                          <div className='text-sm'>
                            {selected.customerNameSnapshot || selected.customerName || '-'}
                          </div>
                          <a
                            className='text-muted-foreground inline-flex items-center gap-1 text-xs'
                            href={
                              selected.customerPhoneSnapshot
                                ? `tel:${selected.customerPhoneSnapshot}`
                                : undefined
                            }
                            onClick={e => e.stopPropagation()}
                          >
                            <Phone className='h-3 w-3' />
                            {selected.customerPhoneSnapshot || selected.customerPhone || '-'}
                          </a>
                          <div className='text-muted-foreground text-xs'>
                            {selected.customerEmailSnapshot || ''}
                          </div>
                        </div>
                        <div className='space-y-1'>
                          ...
                          <div className='text-muted-foreground text-xs'>
                            Contact: {selected.restaurantContactPerson || '-'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className='grid gap-4 sm:grid-cols-2'>
                        <div className='space-y-1'>
                          ...
                          <div className='text-sm'>
                            {selected.deliveryPartnerName || 'Unassigned'}
                          </div>
                          <a
                            className={cn(
                              'text-muted-foreground inline-flex items-center gap-1 text-xs',
                              !selected.deliveryPartnerPhone && 'pointer-events-none'
                            )}
                            href={
                              selected.deliveryPartnerPhone
                                ? `tel:${selected.deliveryPartnerPhone}`
                                : undefined
                            }
                            onClick={e => e.stopPropagation()}
                          >
                        ...
                        <div className='space-y-1'>
                          ...
                          <div className='text-sm leading-5 whitespace-pre-wrap'>
                            {selected.deliveryAddressSnapshot}
                          </div>
                          <a
                            className='text-muted-foreground inline-flex items-center gap-1 text-xs'
                            href={openMapsUrl(selected)}
                            target='_blank'
                            rel='noreferrer'
                            onClick={e => e.stopPropagation()}
                          >
                  ...
                  <Card className='rounded-xl'>
                    ...
                    <CardContent className='grid grid-cols-2 gap-3'>
                      ...
                      <InfoRow
                        ...
                        value={formatDateTime(selected.expectedDeliveryTime)}
                      />
                      <InfoRow
                        label='Actual Delivery'
                        value={formatDateTime(selected.actualDeliveryTime)}
                      />
                    </CardContent>
                  </Card>
                  ...
                  <Card className='rounded-xl'>
                    ...
                    <CardContent className='space-y-3'>
                      <div className='min-h-[20px] text-sm whitespace-pre-wrap'>
                        {selected.notesToRestaurant || (
                          <span className='text-muted-foreground'>No notes</span>
                        )}
                      </div>
                      {selected.cancellationReason && (
                        <div className='bg-destructive/10 rounded-lg p-3 text-sm'>
                          <div className='mb-1 font-medium'>Cancellation</div>
                          <div className='text-sm'>{selected.cancellationReason}</div>
                          <div className='text-muted-foreground mt-1 text-xs'>
                            By: {selected.cancelledByUserType || '-'}
                          </div>
                        </div>
                      )}
                    </CardContent>
...
function InfoRow({
  ...
  return (
    <div className='flex items-center justify-between gap-2'>
      ...
      <div className='flex items-center gap-2'>
        {icon}
        <div className='max-w-[220px] truncate text-sm' title={String(value)}>
          {String(value)}
        </div>
        {copyable && (
          <Button
...
function MoneyRow({
  ...
  return (
    <div className='flex items-center justify-between'>
      <div className='text-muted-foreground text-xs'>{label}</div>
      <div className={cn('flex items-center gap-1 text-sm', bold && 'font-semibold')}>
        {' '}
        <IndianRupee className='h-3 w-3' />
        {v.toFixed(2)}
      </div>
